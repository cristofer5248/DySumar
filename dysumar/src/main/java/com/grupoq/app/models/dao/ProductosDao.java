...
//	@Query("select p from Producto p join fetch p.proveedor pro join fetch p.marca m join fetch p.categoria c join fetch p.margen mar join fetch p.inventarios i where p.id=?1")
	@Query("select p from Producto p join fetch p.proveedor pro join fetch p.marca m join fetch p.categoria c left join p.inventarios i left join p.productosmodify where p.id=?1")
